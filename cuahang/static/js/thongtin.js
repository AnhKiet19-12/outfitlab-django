...
document.addEventListener('DOMContentLoaded', function() {
    ...
    avatarWrapper.addEventListener('click', function(e) {
        // Không mở menu nếu click vào nút xóa hoặc label chọn file
        if (e.target.closest('#btnDeleteAvatar') || 
            e.target.tagName === 'LABEL' && e.target.getAttribute('for') === 'id_avatar') {
            return;
        }
    ...
    // ====================== ĐỊA CHỈ GIAO HÀNG ======================
    // Kiểm tra tồn tại trước khi chạy (tránh lỗi nếu không có modal)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (document.querySelectorAll('.address-item').length === 0) {
                            ...
                            if (list) list.innerHTML = '<p class="text-muted">Bạn chưa có địa chỉ giao hàng nào.</p>';
                        }
                    }
                });
            });
    ...
    console.log('%c✅ Trang Thông tin cá nhân đã sẵn sàng!', 'color: #ec4899; font-weight: bold;');
});